...
## 2026-07-26 눌림 재상승 v3 지속 검증

- 장중 재생 백테스트를 `minute-replay-v3`로 올려 운영 전략과 동일하게 통제된 눌림 뒤 거래량을 동반한 재상승이 있어야 진입한다.
- 단순 고점 돌파는 더 이상 백테스트 진입으로 인정하지 않는다.
- 과거 시점의 횡단면 순위가 없으므로 현재 거래대금 순위는 선도주 대용치로만 사용하며, 이 선택 편향은 결과에 계속 표시한다.
- 기존 v1/v2 결과는 기준선으로 보존하고 v3 표본과 섞지 않는다.
- AWS에서는 현재 전략 검증과 직접 연결되는 1분봉 워커만 자동 실행한다. 시작 3분 지연, 시장별 1종목씩, 15분 간격과 공용 연구 잠금을 유지한다.
- 일·주·월봉 및 국내장 별도 연구 워커는 서버 안정성을 위해 자동 실행하지 않는다.
...
- 배포 후에도 약 45초 주기의 프로세스 재시작이 확인되어 자동 1분봉 리플레이 스레드는 우선 비활성화했다. 시작 시 v1 원시 거래행은 삭제하고 요약 지표는 보존한다. 실시간 v4 PAPER·보호매도·분석 루프는 계속 실행하며, 과거 리플레이는 서버 안정화 확인 뒤 별도 저부하 작업으로 재개한다.
- 리플레이만 꺼도 재시작이 계속되어 시작 15초의 오프마켓 연구와 20초의 국내 복기 자동 스레드도 함께 격리했다. 1GB VPS 상시 프로세스에는 실시간 분석·포지션 위험 감시·매매일지·알림만 남기고, 대용량 차트 연구는 별도 프로세스/스케줄러로 옮기기 전까지 자동 실행하지 않는다.

# 2026-07-23 상대강도 전술 `relative-strength-v5`

- 7월 22일 메인 PAPER는 한국·미국 모두 0건이었다. v4 SHADOW 최근 40건은 승률 30%, 비용 후 평균 −0.437%, PF 0.31, 최근 8건 0승으로 승격 불가 판정했다.
- 절대등락률과 단일 스냅샷 추격을 폐기했다. 거래대금 상위 50개에서 시장 중앙값보다 한국 +0.5%p, 미국 +0.8%p 이상 강하고 상위 25%에 속한 종목만 관찰한다.
- 최소 4회·25초 동안 상대강도 상위권, 거래대금 증가, 순위 유지, 당일 상승률 비붕괴를 모두 확인해야 `정밀 분석`이 된다. 점수가 높아도 확인 전에는 `관찰`이며 SHADOW 진입도 만들지 않는다.
- v5 표본은 v4와 분리한다. 동일 상황 12건, 승률 40% 이상, 전체·최근 기대값 양수, PF 1.15 이상을 통과해야 메인 PAPER로 승격한다.
- SHADOW 요약에 최근 거래일별 한국·미국 성적을 추가해 다음 브리핑부터 양 시장을 따로 비교할 수 있게 했다.
- −0.5% PAPER 보호매도, +1% 50% 부분익절 후 −0.5% 추적손절, 최대 3포지션, 종목당 최대 30%, 총 미청산 위험 1%는 변경하지 않았다.

# 2026-07-24 가격경로 전면개편 `pullback-resumption-v6`

- v5는 40건에서 승률 27.5%, 비용 후 평균 −0.599%, PF 0.285, 최근 8건 승률 37.5%·평균 −0.463%로 폐기 판정했다. 같은 날 한국장 24건도 승률 25%, 평균 −0.671%, PF 0.278이었고 직전 미국장 13건은 승률 23.1%, 평균 −0.676%, PF 0.154였다.
- 실패 원인은 상대강도 상위 종목을 골라도 이미 오른 가격을 바로 추격했다는 점이다. v6는 점수나 상대강도만으로 진입할 수 없고 `선도주 지속 확인 → 통제된 눌림 → 거래대금 동반 재상승`의 순서를 실제 장중 관측값으로 모두 완료해야 한 번만 진입 신호를 낸다.
- 한국장은 시장 대비 +0.5%p 이상인 상위 25% 선도주에서 고점 대비 0.2~0.8% 눌림과 저점 대비 0.15% 재상승을 요구한다. 미국장은 시장 대비 +0.8%p, 0.3~1.0% 눌림, 0.2% 재상승을 요구한다. 두 시장 모두 거래대금 증가, 거래대금 순위 안정, 최소 25초 가격경로 확인이 필요하다.
- 눌림이 최대폭을 넘거나 시장 대비 강도가 사라지면 해당 사이클을 즉시 초기화한다. `RESUMPTION_CONFIRMED`는 한 스캔에서만 발생하며 다음 눌림 사이클을 새로 완성하지 않으면 같은 상승 흐름으로 재진입하지 않는다.
- v4·v5 SHADOW 표본은 실패 오답노트로 그대로 보존하고, v6의 승률·비용 후 평균·PF 및 한국·미국 분리는 `engineVersion`으로 완전히 분리한다. v6 표본이 동일 상황 최소 12건과 기존 승격 조건을 통과하기 전에는 메인 PAPER 진입이 계속 차단된다.
- 장 종료 감시를 놓쳐 다른 시장의 SHADOW 포지션이 `OPEN`으로 남는 문제를 수정했다. 마지막 관측 가격과 시각을 계속 저장하고, 공식 정규장 종료 또는 거래일 변경을 확인하면 마지막 관측가 기준 `마감청산`으로 복구하며 별도 표시한다.
- 전체 92개 테스트, Python 구문 검사, JavaScript 구문 검사, diff 공백 검사를 통과했다.
- −0.5% PAPER 보호매도, +1% 50% 부분익절 후 −0.5% 추적손절, 최대 3포지션, 종목당 최대 30%, 총 미청산 위험 1%, PAPER 전용 원칙은 변경하지 않았다.

# 2026-07-24 비용·통계 검증 강화 `cost-aware-pullback-v7`

- 전체 447건의 매매일지에서 매매 손익은 약 −11.6만 원이었지만 추정 왕복비용이 약 93.8만 원 누적되어 최종 순손실이 약 −105.4만 원이었다. v6의 진입 타이밍 수정만으로는 부족하므로 과매매와 거짓 승격을 직접 차단하는 검증 계층을 추가했다.
- 동일 상황의 메인 PAPER 승격은 12건에서 30건, 최근 표본은 8건에서 12건으로 강화했다. 최소 2거래일, 비용 후 전체·최근 평균 양수, PF 1.25 이상, 실제 평균 손익으로 계산한 손익분기 승률 통과, 평균 수익률의 90% 신뢰하한 양수를 모두 요구한다.
- 레버리지·인버스 승격은 최소 40건과 2거래일, 손익분기 승률, 90% 신뢰하한, PF 1.30을 별도로 모두 통과해야 한다.
- 거래대금 상위 유니버스의 상승 종목 비율·중앙값·상하위 사분위·분산으로 `확산 상승/선별 상승/하락 우세`를 기록한다. 아직 직접 진입 차단에는 쓰지 않고, 같은 시장·시간대·상품군·등락 구간에 시장 확산 상태까지 더해 SHADOW 성과를 분리한다.
- 모든 신규 메인·SHADOW 포지션은 진입 후 최고가와 최저가를 저장하고 청산 시 MFE(최대 유리 이동)와 MAE(최대 불리 이동)를 기록한다. 이후 진입이 틀렸는지, 수익 구간을 청산이 놓쳤는지 분리해서 판단한다.
- 같은 v7 진입 신호에서 SHADOW 청산을 두 갈래로 동시에 추적한다. 메인 승격 대상은 기존 `+1% 50% 부분익절 후 추적손절`이고, 비교군은 −0.5% 손절을 동일하게 둔 채 비용 후 손익비 1.25를 확보하는 고정 목표(KR 약 +1.3%, US 약 +1.53%) 전량청산이다. 두 결과는 섞지 않고 `experimentVariant`별로 집계한다.
- 전체 94개 테스트, Python/JavaScript 구문 검사, diff 공백 검사를 통과했다.
- −0.5% PAPER 보호매도, 최대 3포지션, 종목당 최대 30%, 총 미청산 위험 1%, PAPER 전용 원칙은 변경하지 않았다.

# 2026-07-30 인터뷰 확정 전략 `box-breakout-retest-v8`

- 한국·미국 정규장 거래대금 상위 30개 일반주만 관찰한다. ETF·ETN·레버리지·인버스와 한국 5,000원/미국 5달러 미만 종목은 제외한다. 미국 데이마켓은 신규 진입 없이 국내장 복기에 사용한다.
- 실시간 10초 스냅샷을 완성 3분봉으로 묶고 15~30분 적응형 박스를 만든다. `돌파봉 거래대금 1.5배 이상 → 박스 상단을 지키는 저거래량 재확인 → 다음 3분봉 상승 마감과 거래대금 재증가`가 순서대로 끝나야 한 번만 신호를 낸다.
- 시장이 하락 우세가 아니고 해당 업종이 강해야 한다. 업종 자료가 없으면 시장 중앙값 대비 한국 +0.5%p/미국 +0.8%p 이상의 상대강도로 대체한다. 점수는 후보 순위에만 쓰며 이 가격·거래량 순서를 우회하지 못한다.
- v8 SHADOW 장부는 v7과 완전히 분리한다. v7은 무자본 비교군으로 계속 기록하되 v8 성과·승격 표본에 섞지 않는다.
- 메인 PAPER 승격은 시장별 v8 60건, 5거래일, 최근 20건을 확보한 뒤 비용 후 전체·최근 평균 양수, PF 1.30 이상, 90% 신뢰하한 양수를 모두 충족해야 한다. 승격 전 메인 주문 0건은 정상 동작이다.
- 매수 체결 즉시 평균 체결가 대비 −0.5% PAPER 보호매도를 등록한다. 시장별 추정 왕복비용을 반영해 순손실 위험의 정확히 2배가 순수익으로 남는 가격(KR 약 +1.9%, US 약 +2.2%)에서 전량 익절한다.
- 진입 뒤 완성 3분봉이 박스 상단 아래로 마감하면 전량 청산하며, 그렇지 않아도 30분 안에 손절·2R 목표에 닿지 못하면 전량 청산한다. 부분익절·추적손절은 v8에서 사용하지 않는다.
- PAPER 기준 거래당 비용 포함 계획손실은 계좌의 0.25%, 전체 동시 계획손실은 0.75%다. 최대 3포지션, 같은 업종 1포지션, 종목당 최대 30%를 강제한다. 과거 저장 설정으로 이 한도가 넓어지지 않는다.
- 같은 종목은 청산 후 최소 30분을 기다리고 새 박스·새 확인봉이 완성된 경우에만 재진입하며 하루 최대 2회다. 전체 시장의 일일 진입 횟수 제한과 일 손익 거래잠금은 두지 않는다.
- 적응형 수정은 SHADOW A/B에서 한 번에 한 변수만 바꾸고 시장별 승격 조건을 다시 통과한 뒤 적용한다. PAPER 전용이며 실제 주문 API는 연결하지 않는다.
- 전체 102개 테스트, Python 구문 검사, JavaScript 구문 검사, diff 공백 검사를 통과해야 배포한다.

# 2026-07-30 연구 엔진 별도 프로세스 분리

- 1GB Lightsail의 메인 `toss.service`에서는 휴장 연구 스레드를 전부 비활성화했다. 실시간 분석·PAPER 보호주문·매매일지·알림만 장기 실행 프로세스에 남긴다.
- `toss-research.timer`가 10분마다 `toss-research.service`를 일회성으로 실행한다. 연구 프로세스는 CPU 45%, 메모리 384MB부터 제한·600MB 강제 상한, 낮은 우선순위로 운영되며 중복 실행 잠금이 있다.
